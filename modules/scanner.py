...
from log import info, error, warning, success, println, banner, print_coloured, colours, bcolours
try:
...
class TargetInfo:
    ...
    def coloured(self):
        return (
            (
                bcolours.yellow + "MAC Address : " + bcolours.endc + "{0:20}" +
                bcolours.yellow + " Vendor : " + bcolours.endc + "{1:30}" + "\n" +
                bcolours.yellow + "OS : " + bcolours.endc + "{2:20}" +
                bcolours.yellow + " Accuracy : " + bcolours.endc + "{3:5}" +
                bcolours.yellow + " Type : " + bcolours.endc + "{4:20}" + "\n"
            ).format(
                    str(self.mac),
                    str(self.vendor),
                    str(self.os[:20]),
...
def PortScan(target, scanspeed=5, mode=ScanMode.Normal, customflags=""):
    banner("Running a portscan on host " + str(target) + "...", colours.green)
...
def NoiseScan(target, scantype=ScanType.ARP, timeout=None):
    banner("Creating noise...", colours.green)
...
def DiscoverHosts(target, scantype=ScanType.ARP, scanspeed=3, mode=ScanMode.Normal):
    if type(target) is list:
        banner("Scanning " + str(len(target)) + " target(s) using " + str(scantype.name) + " scan...", colours.green)
    else:
        banner("Scanning " + target + " using " + str(scantype.name) + " scan...", colours.green)
...
def AnalyseScanResults(nm, target=None):
    ...
    println(CurrentTargetInfo.coloured().center(60))
    ...
    for port in nm[target]['tcp'].keys():
        ...
        println(
            (
                bcolours.cyan + "Port : " + bcolours.endc + "{0:10}" + 
                bcolours.cyan + " State : " + bcolours.endc + "{1:10}" +
                bcolours.cyan + " Service : " + bcolours.endc + "{2:15}" +
                bcolours.cyan + " Product : " + bcolours.endc + "{3:20}" +
                bcolours.cyan + " Version : " + bcolours.endc + "{4:15}"
