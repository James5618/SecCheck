...
def SendEmail(email,password, email_to, email_from, server, port, attachment):
    ...
    mail.quit()
    print_coloured("Email report sent successfully", colours.green)
...
def SendWebhook(url, attachment):
    ...
    if response.status_code == 200:
        print_coloured("Webhook report sent successfully", colours.green)
    else:
        print_coloured("Webhook report failed to send", colours.red)
