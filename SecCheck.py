...
try:
    ...
    from modules.log import info, error, warning, success, println, banner, InitializeLogger, print_coloured, colours, bcolours
except ImportError as e:
    print("[!] ImportError: " + str(e))
    exit(1)

__author__ = 'James O`Sullivan'

#=============== Commandline Flags/Arguments that can be used. ========================
argparser = ArgumentParser(description="Server Scanner")
...
def InitArgsConf():
    ...
    try:
        ...
        if config.has_option('SecCheck', 'target'):
            args.target = config.get('SecCheck', 'target').lower()
        if config.has_option('SecCheck', 'hostfile'):
            args.hostfile = config.get('SecCheck', 'hostfile').lower()
        if config.has_option('SecCheck', 'scantype'):
            args.scantype = config.get('SecCheck', 'scantype').lower()
        if config.has_option('SecCheck', 'nmapflags'):
            args.nmapflags = config.get('SecCheck', 'nmapflags').lower()
        if config.has_option('SecCheck', 'speed'):
            try:
                args.speed = int(config.get('SecCheck', 'speed'))
            except ValueError:
                error("[!] Invalid speed value in config file. (Default : 3)")
        if config.has_option('SecCheck', 'apikey'):
            args.api = config.get('SecCheck', 'apikey').lower()
        if config.has_option('SecCheck', 'auto'):
            args.yesplease = True
        if config.has_option('SecCheck', 'mode'):
            args.mode = config.get('SecCheck', 'mode').lower()
        if config.has_option('SecCheck', 'noisetimeout'):
            args.noisetimeout = config.get('SecCheck', 'noisetimeout').lower()
        if config.has_option('REPORT', 'output'):
            args.output = config.get('SecCheck', 'output').lower()
...
def InitArgsAPI():
    if args.api:
        ...
        try:
            ...
            apiKey = None
    return apiKey
#======================Find Package Manager on common Distro to install Nmap ==============================
...
def GetHostsToScan(hosts):
    ...
    index = 0
    for host in hosts:
        println((bcolours.red + "[" + bcolours.endc + str(index) + bcolours.red + "] " + bcolours.endc + host).center(60))
        index += 1
    if DontAskForConfirmation:
        return hosts
    print_coloured("\nEnter the index number of the host you would like to enumurate further.", colours.yellow)
    print_coloured("Enter 'all' to enumurate all hosts.", colours.yellow)
    print_coloured("Enter 'exit' to exit.\n", colours.yellow)
    while True:
        host = input(bcolours.blue + "────> " + bcolours.endc)
        if host == 'all':
            Targets = hosts
            break
        elif host == 'exit':
            ...
            try:
                ...
                print_coloured("Please enter a valid host number or 'all' or 'exit'", colours.red)
